...
1. 打开 https://github.com/new ，新建一个仓库，命名为 `shenyue-math`
...
```bash
cd "E:\MyOutput\AI_Project\申悦学习-AI指导数学学习"
git add .
git commit -m "初始版本：国庆数学专项网站 + 知识库"
git branch -M main
git remote add origin https://github.com/<你的用户名>/shenyue-math.git
...
4. 等待 1~3 分钟，刷新该页面，上方会显示访问地址，形如：
   `https://<你的用户名>.github.io/shenyue-math/`
...
- 数学公式由本地化 KaTeX（`docs/assets/katex/`）在浏览器端渲染，**离线也能显示**，不依赖外部 CDN。
- 若以后想绑定自己的域名（如 `math.example.com`），在 Pages 设置里填域名，
